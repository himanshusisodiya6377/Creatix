        ...
        <div className='flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar'>
          ...
          <div className='flex items-center gap-2 sm:min-w-90 text-nowrap'>
              ...
              <div className='max-w-64 sm:max-w-xs'>
                <p className='text-sm text-medium capitalize trucate'>{project.name}</p>
                <p className='text-xs text-gray-400 -mt-0.5'>Previewing last saved version</p>
              </div>
              <div className='sm:hidden flex-1 flex justify-end'>
                {isMenuOpen ? 
                <MessageSquareIcon onClick={()=> setIsMenuOpen(false)} className='size-6 cursor-pointer' /> : <XIcon onClick={()=> setIsMenuOpen(true)}  className='size-6 cursor-pointer' />}
          ...
          <div className='flex itms-center justify-eSnd gap-3 flex-1 text-xs sm:text-sm'>
            ...
            transition-colors border border-gray-700'>
              {isSaving ? <Loader2Icon size={16} className='animate-spin' /> : <SaveIcon size={16} />} Save
