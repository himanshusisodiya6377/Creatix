...
function analyzeUserInput(title, userPrompt) {
    ...
    // CTR factors
    const ctrFactors = [];
    if (combinedText.includes('shocking') || combinedText.includes('wow'))
        ctrFactors.push('shock value');
    if (combinedText.includes('mystery') || combinedText.includes('secret'))
        ctrFactors.push('curiosity gap');
    if (combinedText.includes('fail') || combinedText.includes('epic'))
        ctrFactors.push('emotional reaction');
    if (combinedText.includes('test') || combinedText.includes('vs'))
        ctrFactors.push('comparison element');
    if (emotionalTone === 'urgency')
        ctrFactors.push('scarcity/urgency');
    return {
...
function generateDynamicThumbnailConcept(title, userPrompt, userStyle, colorScheme, aspectRatio, textOverlay) {
    ...
    const backgroundDescription = colorGuide[colorScheme] || 'dynamic, visually striking color palette';
    ...
    const styleDescription = styleMap[userStyle] || 'visually striking modern style';
...
const generateThumbnailAsync = async (thumbnailId, userId, title, user_prompt, style, aspect_ratio, color_scheme, text_overlay) => {
    try {
