...
export async function generateImageWithPicsArt(params) {
    try {
        if (!PICSART_API_KEY) {
        ...
        const response = await axios.post(PICSART_API_URL, {
            prompt: prompt,
        ...
        while (attempts < maxAttempts) {
            ...
            try {
                ...
                try {
                    ...
                    resultResponse = await axios.get(`${PICSART_API_URL}/${transactionId}`, {
                        headers: {
                            'X-Picsart-API-Key': PICSART_API_KEY,
                        },
                        timeout: 10000,
                    ...
                    if (e1.response?.status === 404) {
                        resultResponse = await axios.get(`${PICSART_RESULT_URL}/${transactionId}`, {
                            headers: {
                                'X-Picsart-API-Key': PICSART_API_KEY,
                            },
                            timeout: 10000,
